...
module.exports.create = function (port, timeout, env, serverId, notCache, closeFlag, systemNanoTime) {
  ...
  var conf = require("./conf")(constants.CONFIG_DIR);
  ...
  // systemNanoTimeを取得.
  var getSystemNanoTime = function() {
    return _SYSTEM_NANO_TIME;
  }

  // 現状の動作環境のコンフィグを取得.
  var getEnvConf = function() {
  ...
  // cbox処理を生成.
  var cbox = require("./cbox").create(isNotCache(), isCloseFlag(), getSystemNanoTime());

  // cboxに対して現在の実行環境コンフィグをセット.
  cbox.setEnvConf(getEnvConf())
